        ...
        if (empty($emailErr) && empty($passwordErr)) {
            ...
            if ($sql = $conn->prepare($sql_code)) {
                ...
                if ($sql->execute()) {
                    ...
                    if ($sql->num_rows == 1) {
                        ...
                        if ($sql->fetch()) {
                            if ($isApproved == 1 || $isApproved == 2) {
                                if (password_verify($password, $password_db)) {
                                    ...
                                    if ($isApproved == 2) {
                                        if ($userverified == 'not verified') {
                                            ...
                                            echo '<script>
                                            modal("Account is still pending. User is unable to reserve until approved",function(){
                                ...
                                echo '<script>
                                modal("Account has been rejected, please try again",function(){
                                    window.location.href="Window_HomePage.php"
                                })
                                     </script>';
                            }
                    ...
                    echo '<script>modal("Oops, something went wrong",function(){return})</script>';
                }
                $sql->close();
            }
            $conn->close();
