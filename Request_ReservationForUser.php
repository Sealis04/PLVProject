...
if ($page)
    ...
    $query = "SELECT COUNT(*) as num FROM tbl_reservation WHERE r_user_ID = ? and r_status = 0 ";
		...
		$sql5->close();
$sql_code = "SELECT * FROM tbl_reservation INNER JOIN tbl_room ON tbl_reservation.r_room_ID = tbl_room.room_ID WHERE tbl_reservation.r_user_ID = ? ORDER BY r_startDateAndTime  DESC LIMIT $start, $limit";
